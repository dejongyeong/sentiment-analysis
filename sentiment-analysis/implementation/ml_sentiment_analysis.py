...
tv_test = cv.transform(testX.ravel())  # transform test review into features

print(f'bag of word model: train: {cv_train.shape} || test: {cv_test.shape}')
...
mnb = MultinomialNB()
mnb.fit(cv_train, trainY)  # build model
cv_pred = mnb.predict(cv_test)  # predict using model
...
tv_metrics = pd.DataFrame([tv_metrics], columns=['accuracy', 'precision', 'recall', 'f1'], index=['tfidf'])
print('Performance Metrics:')
print(tabulate(tv_metrics, headers='keys', tablefmt='github'))

# compare evaluation with different feature extraction method with graph
compare = pd.concat([cv_metrics, tv_metrics])  # concat two dataframs
fig = plt.figure()
ax = compare.plot.bar()
plt.title('MultinomialNB Performance Evaluation Comparison of BOW and TF-IDF\n')
plt.ylabel('result')
...
print('\nConfusion Matrix of MultinomialNB with TF-IDF Feature Extraction:\n')

# display and plot confusion matrix of tf-idf
tv_cm = mt.confusion_matrix(testY, tv_pred, labels=labels)
fig = plt.figure()
ax = fig.add_subplot(111)
plt.title('Confusion Matrix of MultinomialNB with TF-IDF Feature Extraction\n')
...
tv_report = mt.classification_report(testY, cv_pred, labels=labels)
print(tv_report)
